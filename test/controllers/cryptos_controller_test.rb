...
class CryptosControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create crypto" do
    assert_difference('Crypto.count') do
      post cryptos_url, params: { crypto: { abbreviation: @crypto.abbreviation, date: @crypto.date, name: @crypto.name, quantity: @crypto.quantity, value: @crypto.value } }
    end
  ...
  test "should update crypto" do
    patch crypto_url(@crypto), params: { crypto: { abbreviation: @crypto.abbreviation, date: @crypto.date, name: @crypto.name, quantity: @crypto.quantity, value: @crypto.value } }
